...
fn migrate_registry_tool_migrates_v1_to_v2_and_rolls_back() {
    ...
    // 3) --apply --yes: backs up, migrates to V2, verifies.
    ...
    assert_eq!(registry_schema(&data_dir), Some(RegistrySchema::V2Current), "persisted V2");
    ...
    // 4) Re-apply is a safe no-op (already V2).
