...
| `qchain-stark` | **Compresión STARK v1** (Winterfell 0.13): prueba conservación de valor de un lote de `Transfer` — sin range checks, sin enlace a Merkle real, standalone (no conectado a `qchain-execution`/`qchain-node` todavía) |
...
Con fase 2 cerrada (6/6), lo pendiente es: endurecer `qchain-stark` v1 (range checks para no-negatividad real de `u64`, enlace a la raíz Merkle real de `qchain-storage`, conexión a `qchain-execution`/`qchain-node`/RPC para light clients — v2 sin fecha comprometida, no iniciar sin pedido explícito). Además: persistencia real (RocksDB en vez de `InMemoryStore`), regla de commit indirecta de Bullshark, capa de workers de Narwhal, puente EVM/Solana (si hay demanda), calibración económica real de fees/dust/gobernanza (los valores actuales son placeholders explícitos, no cifras modeladas), arreglo real del mempool (orden de nonce/reintento de transacciones perdidas), retransmisión de votos individuales y re-sincronización de certificados perdidos (solo se arregló el reenvío de la propuesta de vértice), y expansión del conjunto de validadores más allá de n=50 (validado empíricamente solo hasta ahí).
