...
Persistencia real (RocksDB en vez de `InMemoryStore`), regla de commit indirecta de Bullshark, capa de workers de Narwhal, compresión STARK (Winterfell), expansión del conjunto de validadores, investigación de agregación lattice-based, puente EVM/Solana (si hay demanda), calibración económica real de fees/dust/gobernanza (los valores actuales son placeholders explícitos, no cifras modeladas), arreglo real del mempool (orden de nonce/reintento de transacciones perdidas), retransmisión de votos individuales y re-sincronización de certificados perdidos (solo se arregló el reenvío de la propuesta de vértice).
