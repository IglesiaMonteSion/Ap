...
**Estado de implementación (actualizado durante la construcción, no una promesa de diseño).** De fase 2, ya construido y probado end-to-end (código real, no maqueta): registro de algoritmos con flujo real de alta/baja vía gobernanza (activar/depreciar/retirar, con supermayoría 2/3 + time-lock de revisión obligatorio) y ejecución de gobernanza automatizada on-chain (sin paso manual/multisig) - ver `qchain-governance` y `qchain-execution`'s `governance.rs`. Esto además cerró retroactivamente un pendiente de fase 1: "delegated staking activo" estaba decidido desde el principio (ver arriba) pero nunca se implementó en el código de fase 1 - se construyó ahora (`staking.rs`) porque gobernanza necesitaba una base de voto ponderado por stake real, no la lista fija de validadores de consenso. Pendiente todavía de fase 2: compresión STARK (Winterfell), testing de simulación determinista, expansión del conjunto de validadores, investigación de agregación lattice-based, y gobernanza de parámetros de bajo riesgo (fee curve) - esto último requiere primero convertir esas constantes en estado on-chain mutable, un incremento aparte. Detalle completo en la skill `project-lessons-learned`.
